...
init: ## Initializes the terraform remote state backend and pulls the correct environments state.
	...
	@terraform remote config \
		...
		-backend-config="bucket=$(lastword $(subst /, ,$(PWD)))-euwest1-terraform-state" \
		-backend-config="key=$(lastword $(subst /, ,$(PWD))).tfstate"
...
apply: init update ## Applies a new state.
	@terraform apply -input=true -refresh=true -var-file=$(lastword $(subst /, ,$(PWD))).tfvars && terraform remote push
...
destroy: init update ## Destroys targets
	@terraform destroy -var-file=$(lastword $(subst /, ,$(PWD))).tfvars && terraform remote push