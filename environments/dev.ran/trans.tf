resource "vsphere_folder" "trans_folder" {
  datacenter = "${var.vsphere_datacenter}"

  path = "${format("%s/TRANS", vsphere_folder.env_folder.path)}"
}
...
module "trans" {
  ...
  #srv_number    = "01"
  env_folder = "${vsphere_folder.trans_folder.path}"
