resource "vsphere_folder" "beanstalk_folder" {
  datacenter = "${var.vsphere_datacenter}"

  path = "${format("%s/BEANSTALK", vsphere_folder.env_folder.path)}"
}
...
module "beanstalk" {
  ...
  #srv_number    = "01"
  env_folder = "${vsphere_folder.beanstalk_folder.path}"
