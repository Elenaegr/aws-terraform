resource "vsphere_folder" "lbvoice_folder" {
  datacenter = "${var.vsphere_datacenter}"

  path = "${format("%s/LBVOICE", vsphere_folder.env_folder.path)}"
}
...
module "lbvoice" {
  ...
  #srv_number    = "01"
  env_folder = "${vsphere_folder.lbvoice_folder.path}"
